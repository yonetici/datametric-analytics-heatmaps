...
(function($) {
	...
	$(document).ready(function() {
		if (parseInt(datametricPublicData.is_admin) === 1) {
	...
	function loadPageStats() {
		$.ajax({
			url: datametricPublicData.rest_url + '/page-stats',
			method: 'GET',
			data: { url: datametricPublicData.page_url },
			beforeSend: function(xhr) {
				xhr.setRequestHeader('X-WP-Nonce', datametricPublicData.rest_nonce);
			},
	...
	function fetchHeatmapData() {
		$.ajax({
			url: datametricPublicData.rest_url + '/clicks',
			method: 'GET',
			data: { url: datametricPublicData.page_url },
			beforeSend: function(xhr) {
				xhr.setRequestHeader('X-WP-Nonce', datametricPublicData.rest_nonce);
	...
	function startSelectorMode() {
		isSelectorMode = true;
		window.datametricSelectorModeActive = true; // Block click tracker global logging
		$('#trackly-stats-panel').removeClass('active');

		$('body').css('cursor', 'crosshair');

		$(document).on('mouseover.datametricSelector', handleSelectorMouseOver);
		$(document).on('mouseout.datametricSelector', handleSelectorMouseOut);
		$(document).on('click.datametricSelector', handleSelectorClick);
	}
	...
	function handleSelectorClick(e) {
		...
		const selector = window.datametricGetUniqueSelector(e.target);
	...
	function exitSelectorMode() {
		...
		window.datametricSelectorModeActive = false; // Re-enable click tracker
		$('body').css('cursor', 'default');
		
		$(document).off('mouseover.datametricSelector');
		$(document).off('mouseout.datametricSelector');
		$(document).off('click.datametricSelector');
	}
	...
	function saveCustomEvent() {
		...
		$.ajax({
			url: datametricPublicData.rest_url + '/save-event',
			method: 'POST',
